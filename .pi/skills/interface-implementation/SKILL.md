...
- Define the implementation as an **exported class**.
- Export a `createXxx()` factory function as a convenience wrapper.
...
1. Re-exports the class and factory function(s) from implementation files.
...
import { createInMemorySessionManager, InMemorySessionManager } from "./in-memory-session-manager.js";

export { createInMemorySessionManager, InMemorySessionManager } from "./in-memory-session-manager.js";
...
export namespace SessionManager {
  ...
  // export function sqlite(path: string): SqliteSessionManager { ... }
}
```
...
Export the class and namespace from the implementations index. A single `export { Xxx }` is enough for the namespace — TypeScript carries both the type (via the merged interface) and the value (the namespace) through one export. The class gets its own named export alongside.
...
// Add the namespace export (carries both type and factory value)
// and the class export:
export {
...
```typescript
import { SessionManager, InMemorySessionManager } from "@otter-agent/core";

// As a type:
function accepts(sm: SessionManager) { ... }

// As a factory (recommended convenience API):
const sm = SessionManager.inMemory();

// Direct class construction (for subclassing, instanceof, or impl-specific APIs):
const sm = new InMemorySessionManager();
```

Both the namespace factory and direct construction produce the same class instance, so `instanceof` works regardless of how the instance was created:

```typescript
const sm1 = SessionManager.inMemory();
const sm2 = createInMemorySessionManager();
const sm3 = new InMemorySessionManager();

sm1 instanceof InMemorySessionManager; // true
sm2 instanceof InMemorySessionManager; // true
sm3 instanceof InMemorySessionManager; // true
```
...
- Direct class construction via `new` produces a working instance
- `instanceof` checks pass for all creation paths (direct, factory, namespace)

## Reference Implementation

See `src/session-managers/` for the canonical worked example:
- `in-memory-session-manager.ts` — exported class + factory
- `index.ts` — namespace merging + class re-export
- `in-memory-session-manager.test.ts` — tests covering all interface methods, edge cases, factory, direct construction, and instanceof
